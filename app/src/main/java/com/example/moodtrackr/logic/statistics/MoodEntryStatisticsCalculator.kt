...
class MoodEntryStatisticsCalculator() {
    ...
    fun calculateStats(moodEntries: List<MoodEntry>): List<IndicatorStats> {
        ...
        for (fieldName in indicatorListNames) {
            ...
            if (getter != null) {
                val entriesForIndicator = moodEntries.mapNotNull { entry ->
